...
const reflushList = function () {
  http.post('/api/user/list', {"current_page": currentPage.value, "page_size": 10}).then((res: any) => {
    userList.length = 0
...
const submit = function () {
  if (editModel.value === 'edit') {
    ...
    http.post('/api/user/edit', newData).then((res: any) => {
      ElNotification({
    ...
    http.post('/api/user/create', newData).then((res: any) => {
      ElNotification({
        title: res.errorNo === 0 ? lang.succ : lang.fail,
        message: res.errorNo === 0 ? "" : res.data,
        type: res.errorNo === 0 ? 'success' : 'error',
