 ...
 * 初始化向导 API 服务层
 *
 * 从旧前端 fe/src/services/setupService.ts 迁移。
 * 封装系统初始化设置（Setup）相关的 HTTP 请求。
 * 涵盖数据库、密码、域名、DNS、SSL 等配置步骤的读取与保存。
 *
 * 核心协议：所有步骤共享 POST /api/setup 端点，
 * 通过 action（get/set/getParams）+ step（database/password/domain/dns/ssl）区分操作。
 *
 * 修改日期: 20260609
 * 修改原因: 旧版使用了不存在的 RESTful 分离端点，导致引导全部失败。
 *           改为与后端 setup.go 控制器一致的统一端点协议。
 * @module setupService
 */
 ...
 * 保存 SSL 配置
 * @param token - Setup 鉴权 Token
 * @param sslType - SSL 类型（0=自动HTTP、1=手动、2=自动DNS）
