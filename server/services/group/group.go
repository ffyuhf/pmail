...
func DelGroup(ctx *context.Context, groupId int) (bool, error) {
	...
	// 修改日期: 20260610 — #16 修复 UPDATE user_email 缺少 user_id，防止影响其他用户的邮件
	_, err = trans.Exec(db.WithContext(ctx, fmt.Sprintf("update user_email set group_id=0 where group_id in (%s) and user_id=?", array.Join(allGroupIds, ","))), ctx.UserID)
	if err != nil {
		trans.Rollback()
		return false, errors.Wrap(err)
	}
