...
// 修改日期: 20260610 — #13 修复 deleteOne 使用 email_id 会导致跨文件夹影响
// 改为先查到具体的 user_email 记录，再按 user_email.id 精确操作，避免影响同用户其他文件夹中的副本
func deleteOne(ctx *context.Context, session *xorm.Session, id int64, forcedDel bool) error {
	// 先查找属于当前用户的第一条 user_email 记录（该 email_id 对应的）
	var ue models.UserEmail
	_, err := session.Table(&models.UserEmail{}).Where("email_id=? and user_id=?", id, ctx.UserID).Get(&ue)
	if err != nil {
		return err
	}
	if ue.ID == 0 {
	...
	if !forcedDel {
		// 软删除：仅更新这一条 user_email 记录，避免跨文件夹影响
		_, err := session.Table(&models.UserEmail{}).Where("id=?", ue.ID).Update(map[string]interface{}{
			"status":   consts.EmailStatusDel,
			"group_id": 0,
		})
		return err
	}
	// 硬删除：先删除关联关系（仅删除这一条 user_email 记录）
	_, err = session.Table(&models.UserEmail{}).Where("id=?", ue.ID).Delete(&models.UserEmail{})
	if err != nil {
		return err
	}
	// 检查 email 是否还有其他 user_email 关联
...
// 修改日期: 20260610 — #5 修复 DelByUID 缺少事务 Begin，确保删除操作的原子性
