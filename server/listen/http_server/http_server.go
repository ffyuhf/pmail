...
import (
	...
	"github.com/ffyuhf/pmail/controllers/email"
...
func router(mux *http.ServeMux) {
	...
	// API Token 管理：生成和撤销随机不透明令牌，替代旧的密码派生 Token
	mux.HandleFunc("/api/token/generate", contextIterceptor(controllers.GenerateAPIToken))
	mux.HandleFunc("/api/token/revoke", contextIterceptor(controllers.RevokeAPIToken))
}
