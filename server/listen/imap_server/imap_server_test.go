...
import (
	...
	pcontext "github.com/ffyuhf/pmail/utils/context"
...
func TestCapability(t *testing.T) {
	...
	// BUG-1 修复验证：IMAP4rev1 基础能力必须存在
	if _, ok := res["IMAP4rev1"]; !ok {
		t.Error("Capability Error: IMAP4rev1 not found")
	}

	// 已认证后再次查询能力，验证扩展能力是否公布
	res2, err := clientLogin.Capability().Wait()
	if err != nil {
		t.Error(err)
	}
	// BUG-1 修复验证：IDLE 能力必须公布
	if _, ok := res2["IDLE"]; !ok {
		t.Error("Capability Error: IDLE not found after login")
	}
	// BUG-4 修复验证：NAMESPACE 能力必须公布
	if _, ok := res2["NAMESPACE"]; !ok {
		t.Error("Capability Error: NAMESPACE not found after login")
	}
	// BUG-5 修复验证：UIDPLUS 能力必须公布
	if _, ok := res2["UIDPLUS"]; !ok {
		t.Error("Capability Error: UIDPLUS not found after login")
	}
	// BUG-6 修复验证：MOVE 能力必须公布
	if _, ok := res2["MOVE"]; !ok {
		t.Error("Capability Error: MOVE not found after login")
	}
	// BUG-7 修复验证：CHILDREN 能力必须公布
	if _, ok := res2["CHILDREN"]; !ok {
		t.Error("Capability Error: CHILDREN not found after login")
	}
