...
type Session struct {
	...
	// 阶段二 P1（RFC 3461）：DSN 投递状态通知参数，仅在客户端请求 DSN 时填充。
	// DSNRcptOpts 记录每个收件人的 NOTIFY/ORCPT 参数，key 为收件人地址。
	DSNRcptOpts map[string]*smtp.RcptOptions
	// DSMailOpts 记录 MAIL FROM 的 RET/ENVID 参数，用于控制退信报告内容范围。
	DSMailOpts *smtp.MailOptions
}
...
// Mail 处理 SMTP MAIL FROM 命令，记录发件人地址和 DSN RET/ENVID 参数。
func (s *Session) Mail(from string, opts *smtp.MailOptions) error {
	log.SmtpInfof(s.Ctx, log.EventSMTPMailFrom, "发件人=%s", from)
	s.From = from
	// 阶段二 P1：记录 MAIL FROM 的 DSN 选项（RET=FULL/HDRS, ENVID=xxx）
	s.DSMailOpts = opts
	return nil
}

// Rcpt 处理 SMTP RCPT TO 命令，记录收件人地址和 DSN NOTIFY/ORCPT 参数。
func (s *Session) Rcpt(to string, opts *smtp.RcptOptions) error {
	log.SmtpInfof(s.Ctx, log.EventSMTPRcptTo, "收件人=%s", to)
	s.To = append(s.To, to)
	// 阶段二 P1：记录收件人的 DSN 通知偏好（NOTIFY=SUCCESS,FAILURE 等）
	if opts != nil && (len(opts.Notify) > 0 || opts.OriginalRecipient != "") {
		if s.DSNRcptOpts == nil {
			s.DSNRcptOpts = make(map[string]*smtp.RcptOptions)
		}
		s.DSNRcptOpts[to] = opts
	}
	return nil
}

// Reset 清理会话状态，保留连接和认证信息（RFC 5321 Section 4.1.4）。
